document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-split').addEventListener('click', async () => {
        ...
        btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Разделение...';
        ...
        try {
            const r = await fetch('/api/tool/pdf-split/process', { method: 'POST', headers: {'X-CSRF-TOKEN': getCsrfToken()}, body: fd });
            const data = await r.json();
            document.getElementById('progress-bar').style.width = '100%';

            if (data.success) {
                document.getElementById('result-stats').textContent = `${data.total_pages} страниц`;
                let html = '';
                data.files.forEach(f => {
                    html += `<div class="result-item"><div><div class="result-item-name">Страница ${f.page}</div><div class="result-item-info">${formatSize(f.size)}</div></div><a href="#" onclick="forceDownload('${f.download_url}', 'page_${f.page}.pdf'); return false;" class="btn btn-secondary" style="padding:8px 16px;font-size:0.85rem;">Скачать</a></div>`;
                });
        ...
        btn.disabled = false; btn.innerHTML = '✂️ Разделить PDF';
    });
});
