document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-process').addEventListener('click', async () => {
        ...
        btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Извлечение...';
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                const r = await fetch('/api/tool/pdf-to-excel/process', { method: 'POST', body: fd, headers: {'X-CSRF-TOKEN': getCsrfToken()} });
        ...
        btn.disabled = false; btn.innerHTML = '📊 Извлечь в Excel';
    });
});
