document.addEventListener('DOMContentLoaded', () => {
    const uploader = new Uploader({
        ...
        accepts: ['image/jpeg', 'image/png', 'image/webp'], maxFiles: 20, supportsPaste: true
    });
    ...
    sizeInput.addEventListener('input', () => document.getElementById('watermark-size-val').textContent = sizeInput.value + 'px');

    document.getElementById('btn-process').addEventListener('click', async () => {
        const files = uploader.getFiles();
        const text = document.getElementById('watermark-text').value.trim();
        if (!files.length) return showToast('Добавьте файлы', 'error');
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            fd.append('text', text);
            fd.append('color', document.getElementById('watermark-color').value);
            fd.append('size', sizeInput.value);
            fd.append('_token', getCsrfToken());

            try {
                const r = await fetch('/api/tool/image-watermark/process', { method: 'POST', body: fd, headers: {'X-CSRF-TOKEN': getCsrfToken()} });
                ...
                if (data.success) {
                    ...
                    html += `<div class="result-item"><span class="result-item-name">❌ Ошибка</span><span class="result-item-info">${data.error}</span></div>`;
                }
            } catch (e) {
        ...
        btn.disabled = false; btn.innerHTML = '©️ Добавить знак';
    });
});
