document.addEventListener('DOMContentLoaded', () => {
    const uploader = new Uploader({
        ...
        accepts: ['image/jpeg', 'image/png', 'image/webp'], maxFiles: 5, supportsPaste: true,
    ...
    document.getElementById('btn-process').addEventListener('click', async () => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                const r = await fetch('/api/tool/image-remove-bg/process', { method: 'POST', body: fd, headers: {'X-CSRF-TOKEN': getCsrfToken()} });
        ...
        btn.disabled = false; btn.innerHTML = '✨ Удалить фон';
    });
});
